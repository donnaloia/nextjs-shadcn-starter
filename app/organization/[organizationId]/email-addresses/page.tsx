...
export default function EmailAddressesPage({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(API_ENDPOINTS.emailAddresses.list(organizationId, currentPage), {
          headers: {
              ...
              <Dialog open={open} onOpenChange={setOpen}>
                ...
                <DialogContent className="w-[600px] max-w-[90vw] p-0">
                  <DialogHeader className="space-y-4 px-16 py-12">
                    <DialogTitle className="text-3xl font-medium">Create Email Address</DialogTitle>
                  ...
                  <form action={onSubmit} className="space-y-8 px-16 pb-12">
                    <div className="space-y-4">
                      <Label htmlFor="name" className="text-xl">Email Address</Label>
                      <Input 
                        id="name" 
                        name="name" 
